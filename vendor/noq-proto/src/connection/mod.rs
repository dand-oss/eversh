...
impl Connection {
    ...
    fn scheduling_info(&self, path_id: PathId) -> PathSchedulingInfo {
        #[cfg(feature = "single-path-scheduling")]
        if self.paths.len() == 1 && path_id == PathId::ZERO && !self.is_handshaking() {
            let path_data = self.path_data(path_id);
            if let Some(info) = single_path_scheduling_info(
                1,
                path_id,
                false,
                path_data.validated,
    ...
    fn poll_transmit_on_path(
        ...
        for space_id in SpaceId::iter() {
            ...
            #[cfg(feature = "skip-discarded-spaces")]
            if space_id != SpaceId::Data
                && should_skip_discarded_space(
                    space_id,
                    self.crypto_state.has_keys(space_id.encryption_level()),
                    self.spaces[space_id].for_path(path_id).loss_probes,
    ...
    fn poll_transmit_path_space(
        ...
        loop {
            ...
            if path_id == PathId::ZERO {
                use crate::diagnostic::{self, NumberSpace};
                let number_space = match space_id {
                    SpaceId::Initial => NumberSpace::Initial,
                    SpaceId::Handshake => NumberSpace::Handshake,
                    SpaceId::Data => NumberSpace::Data,
                };
                diagnostic::emit_packet_frames(builder.packet_number, number_space, true);
            }
            ...
            if path_id == PathId::ZERO {
                ...
                diagnostic::emit_packet_frames(builder.packet_number, number_space, false);
            }
            ...
            #[cfg(feature = "ack-inline-storage")]
            for (path_id, _) in builder.sent_frames().largest_acked.iter() {
                self.spaces[space_id]
                    .for_path(*path_id)
                    .pending_acks
                    .acks_sent();
            ...
            #[cfg(not(feature = "ack-inline-storage"))]
    ...
    #[cfg(all(test, feature = "skip-discarded-spaces"))]
    pub(crate) fn assert_discarded_space_noop(&mut self, now: Instant) {
        for space_id in [SpaceId::Initial, SpaceId::Handshake] {
            assert!(!self.crypto_state.has_keys(space_id.encryption_level()));
            assert_eq!(self.spaces[space_id].for_path(PathId::ZERO).loss_probes, 0);
            for flags in 0u8..16 {
                let scheduling = PathSchedulingInfo {
                    is_abandoned: false,
                    may_send_data: flags & 1 != 0,
                    may_send_close: flags & 2 != 0,
                    may_self_abandon: flags & 4 != 0,
                };
                let before = (
                    format!("{:?}", self.spaces[space_id].pending),
                    self.spaces[space_id]
                ...
                let mut transmit =
                    TransmitBuf::new(&mut buffer, NonZeroUsize::new(1).unwrap(), 1200);
                let remote_cid = self.remote_cids.get(&PathId::ZERO).unwrap().active();
                // Invoke the original path directly, bypassing the new shortcut.
                let result = self.poll_transmit_path_space(
                    now,
                    &mut transmit,
                    PathId::ZERO,
                    space_id,
    ...
    pub(crate) fn handle_first_packet(
        ...
        if let Some(data) = remaining {
            // Initial acceptance happens after endpoint-owned decryption, not
            // through the established-connection receive-cookie path. Its
            // plaintext length excludes the authentication tag; do not invent
            // encrypted offsets or inherit a caller's unrelated context.
            #[cfg(feature = "packet-diagnostics")]
            crate::diagnostic::without_context(|| {
                self.handle_coalesced(now, network_path, path_id, ecn, data, 0);
            });
    ...
    fn handle_coalesced(
        ...
        ecn: Option<EcnCodepoint>,
        data: BytesMut,
        #[cfg(feature = "packet-diagnostics")] datagram_offset: usize,
    ) {
        self.path_data_mut(path_id)
        ...
        #[cfg(feature = "packet-diagnostics")]
        let mut packet_offset = datagram_offset;
        while let Some(data) = remaining {
            match PartialDecode::new(
                ...
                Ok((partial_decode, rest)) => {
                    #[cfg(feature = "packet-diagnostics")]
                    let packet_len = partial_decode.len();
                    remaining = rest;
                    self.handle_decode(
                        now,
                        network_path,
    ...
    fn handle_decode(
        ...
        #[cfg(feature = "packet-diagnostics")] packet_offset: usize,
    ) {
        let packet_len = partial_decode.len();
        let qlog = QlogRecvPacket::new(packet_len);
    ...
    fn handle_packet(
        ...
        #[cfg(feature = "packet-diagnostics")] packet_offset: usize,
        #[cfg(feature = "packet-diagnostics")] packet_len: usize,
    ) {
        ...
        let result = match decrypted {
            ...
            Ok((packet, pn)) => {
                ...
                #[cfg(feature = "packet-diagnostics")]
                // The diagnostic schema intentionally has no path identity. Suppress
                // multipath packets rather than attributing them to PathId::ZERO.
                if path_id == PathId::ZERO
                    && let Some(packet_number) = pn
                {
                    use crate::diagnostic::{self, NumberSpace};
                    let number_space = match packet.header.space() {
                        SpaceKind::Initial => NumberSpace::Initial,
                        SpaceKind::Handshake => NumberSpace::Handshake,
                        SpaceKind::Data => NumberSpace::Data,
                    };
                    diagnostic::emit_packet_authenticated(
                        packet_number,
                        number_space,
                        packet_offset,
                        packet_len,
                ...
                #[cfg(feature = "packet-diagnostics")]
                if path_id != PathId::ZERO {
                    crate::diagnostic::emit_unsupported_path(path_id.0);
                }
        ...
        for result in frame::Iter::new(payload)? {
            ...
            match frame {
                ...
                Frame::Stream(frame) => {
                    ...
                    let should_transmit =
                        self.streams.received(frame, payload_len)?.should_transmit();
                    #[cfg(feature = "packet-diagnostics")]
                    if path_id == PathId::ZERO {
                        use crate::diagnostic::{self, NumberSpace};
                        diagnostic::emit_stream_received(
                            number,
                            NumberSpace::Data,
                            stream_event.0,
                            stream_event.1,
    ...
    fn populate_packet<'a, 'b>(
        ...
        if !scheduling_info.is_abandoned && scheduling_info.may_send_data {
            #[cfg(feature = "ack-inline-storage")]
            let path_ids = if space.number_spaces.len() == 1 {
                None
            } else {
                Some(
                    space
            ...
            #[cfg(feature = "ack-inline-storage")]
            if space.number_spaces.len() == 1
                && let Some((&path_id, pns)) = space.number_spaces.iter_mut().next()
        ...
        if !scheduling_info.is_abandoned
            && scheduling_info.may_send_data
            && take_pending_ack_frequency(
                &mut space.pending.ack_frequency,
        ...
        // NEW_CONNECTION_ID
        // Empty queues cannot emit this frame. The opt-in experiment avoids
        // scanning every local CID solely to size a frame that cannot be sent.
        if !cfg!(feature = "lazy-cid-preparation") || !space.pending.new_cids.is_empty() {
            let cid_len = self
                .local_cid_state
                .values()
                .map(|cid_state| cid_state.cid_len())
...
/// Whether a packet-number space can be skipped before entering its transmit path.
///
/// This is deliberately narrower than `space_can_send`: a space is skipped only after its
/// encryption keys have been discarded, it has no outstanding loss probe, and the path is not
/// abandoned. In particular, keeping the loss-probe and abandoned-path guards here preserves
/// recovery and PATH_ABANDON scheduling semantics.
#[cfg(feature = "skip-discarded-spaces")]
fn should_skip_discarded_space(
    space_id: SpaceId,
    has_keys: bool,
    loss_probes: u32,
    is_abandoned: bool,
) -> bool {
    space_id != SpaceId::Data && !has_keys && loss_probes == 0 && !is_abandoned
}
...
/// Return the scheduling result for the common established, validated, single-primary-path
/// case. Every other state must use the general path comparison below.
#[cfg(feature = "single-path-scheduling")]
fn single_path_scheduling_info(
    path_count: usize,
    path_id: PathId,
    is_handshaking: bool,
    validated: bool,
...
impl AckedPaths {
    ...
    fn insert(&mut self, path_id: PathId, packet_number: u64) {
        match self {
            Self::Empty => {
                *self = Self::One {
                    path_id,
            ...
            } if *current_path == path_id => *current_packet = packet_number,
            Self::One {
                path_id: current_path,
                packet_number: current_packet,
            } => {
                let mut paths = FxHashMap::default();
                paths.insert(*current_path, *current_packet);
                paths.insert(path_id, packet_number);
                *self = Self::Many(paths);
            }
            Self::Many(paths) => {
                paths.insert(path_id, packet_number);
            }
    ...
    fn iter(&self) -> AckedPathsIter<'_> {
        match self {
            Self::Empty => AckedPathsIter {
                one: None,
                many: None,
            },
            Self::One {
                ...
                one: Some((path_id, packet_number)),
                many: None,
            },
            Self::Many(paths) => AckedPathsIter {
                one: None,
                many: Some(paths.iter()),
            },
        }
    }

    #[cfg(test)]
    fn is_inline_single_path(&self) -> bool {
        matches!(self, Self::One { .. })
    }
}

#[cfg(feature = "ack-inline-storage")]
struct AckedPathsIter<'a> {
    one: Option<(&'a PathId, &'a u64)>,
    many: Option<std::collections::hash_map::Iter<'a, PathId, u64>>,
}

#[cfg(feature = "ack-inline-storage")]
impl<'a> Iterator for AckedPathsIter<'a> {
    type Item = (&'a PathId, &'a u64);

    fn next(&mut self) -> Option<Self::Item> {
        self.one.take().or_else(|| self.many.as_mut()?.next())
    }
}

#[cfg(feature = "ack-inline-storage")]
type LargestAcked = AckedPaths;

#[cfg(not(feature = "ack-inline-storage"))]
type LargestAcked = FxHashMap<PathId, u64>;
...
/// Do not consume the request (or subsequently advance its sequence number)
/// until a complete ACK_FREQUENCY frame can fit in the current packet.
fn take_pending_ack_frequency(pending: &mut bool, available: usize) -> bool {
    available >= frame::AckFrequency::SIZE_BOUND && mem::replace(pending, false)
}
...
mod tests {
    ...
    #[cfg(feature = "skip-discarded-spaces")]
    #[test]
    fn skip_discarded_spaces_guard_is_exhaustive() {
        for space_id in SpaceId::iter() {
            for has_keys in [false, true] {
                for loss_probes in [0, 1] {
                    for is_abandoned in [false, true] {
                        let actual = should_skip_discarded_space(
                            space_id,
                            has_keys,
                            loss_probes,
                            is_abandoned,
                        );
                        let expected = space_id != SpaceId::Data
                            && !has_keys
                            && loss_probes == 0
                            && !is_abandoned;
                        assert_eq!(
                            actual, expected,
                            "space={space_id:?} has_keys={has_keys} loss_probes={loss_probes} abandoned={is_abandoned}"
                        );
                    }
                }
            }
    ...
    #[cfg(feature = "single-path-scheduling")]
    #[test]
    fn single_path_scheduling_exhaustive_guard_and_formula_equivalence() {
        for mask in 0u8..64 {
            let paths = if mask & 1 == 0 { 1 } else { 2 };
            let path = if mask & 2 == 0 {
                PathId::ZERO
            } else {
                PathId(1)
            };
            let handshaking = mask & 4 != 0;
            let validated = mask & 8 != 0;
            let abandoned = mask & 16 != 0;
            let has_cids = mask & 32 != 0;
            let actual = single_path_scheduling_info(
                paths,
                path,
                handshaking,
                validated,
            ...
            let eligible = paths == 1
                && path == PathId::ZERO
                && !handshaking
                && validated
                && !abandoned
                && has_cids;
            assert_eq!(actual.is_some(), eligible, "guard mask {mask}");
            if let Some(actual) = actual {
                for status in [PathStatus::Available, PathStatus::Backup] {
                    // Evaluate the existing general formula for its sole path.
                    let have_validated = has_cids && !abandoned && validated;
                    let have_available = have_validated && status == PathStatus::Available;
                    let send = has_cids
                        && !abandoned
                        && (handshaking
                            || (validated && (status == PathStatus::Available || !have_available)));
                    let close = has_cids && !abandoned && (validated || !have_available);
                    let abandon = has_cids && validated && !have_validated;
                    assert_eq!(
                        (
                            actual.is_abandoned,
                            actual.may_send_data,
                            actual.may_send_close,
                            actual.may_self_abandon
                        ),
                        (abandoned, send, close, abandon)
                    );
                }
            }
        }
    ...
    #[cfg(feature = "single-path-scheduling")]
    #[test]
    fn single_path_scheduling_shortcut_matches_stable_path_guards() {
        for status in [PathStatus::Available, PathStatus::Backup] {
            // With one validated, non-abandoned path, the general formula produces the
            // same result for either local status: Available has no better path, while
            // Backup has no available path to prefer.
            let expected = match status {
                PathStatus::Available | PathStatus::Backup => (false, true, true, false),
            };
            let eligible = single_path_scheduling_info(1, PathId::ZERO, false, true, false, true);
            let Some(eligible) = eligible else {
                panic!("stable primary path must use the shortcut");
            };
            assert_eq!(
                (
                    eligible.is_abandoned,
                    eligible.may_send_data,
                    eligible.may_send_close,
                    eligible.may_self_abandon,
                ),
                expected
            );
        }

        for (paths, path_id, handshaking, validated, abandoned, has_cids) in [
            (0, PathId::ZERO, false, true, false, true),
            (2, PathId::ZERO, false, true, false, true),
            (1, PathId(1), false, true, false, true),
            (1, PathId::ZERO, true, true, false, true),
            (1, PathId::ZERO, false, false, false, true),
            (1, PathId::ZERO, false, true, true, true),
            (1, PathId::ZERO, false, true, false, false),
        ] {
            for status in [PathStatus::Available, PathStatus::Backup] {
                assert!(
                    single_path_scheduling_info(
                        paths,
                        path_id,
                        handshaking,
                        validated,
                    ...
                    .is_none(),
                    "shortcut must fall back for paths={paths}, path_id={path_id:?}, handshaking={handshaking}, validated={validated}, abandoned={abandoned}, has_cids={has_cids}, status={status:?}"
                );
            }
        }
    }
    ...
    #[test]
    fn ack_frequency_short_packet_preserves_pending_request() {
        let mut pending = true;
        for available in 0..frame::AckFrequency::SIZE_BOUND {
            assert!(!take_pending_ack_frequency(&mut pending, available));
            assert!(pending, "short packet must not consume ACK_FREQUENCY");
        }
        assert!(take_pending_ack_frequency(
            &mut pending,
            frame::AckFrequency::SIZE_BOUND
        ));
        assert!(!pending);
        assert!(!take_pending_ack_frequency(&mut pending, usize::MAX));
    }

    #[cfg(feature = "ack-inline-storage")]
    #[test]
    fn acked_paths_empty_and_single_path_stay_inline() {
        let mut paths = AckedPaths::default();
        assert!(paths.is_empty());
        assert_eq!(paths.iter().count(), 0);

        paths.insert(PathId::ZERO, 7);
        assert!(!paths.is_empty());
        assert!(paths.is_inline_single_path());
        assert_eq!(paths.iter().collect::<Vec<_>>(), vec![(&PathId::ZERO, &7)]);
    }

    #[cfg(feature = "ack-inline-storage")]
    #[test]
    fn acked_paths_replaces_single_path_value_without_growing() {
        let mut paths = AckedPaths::default();
        paths.insert(PathId::ZERO, 7);
        paths.insert(PathId::ZERO, 11);
        paths.insert(PathId::ZERO, 3);

        assert!(paths.is_inline_single_path());
        assert_eq!(paths.iter().collect::<Vec<_>>(), vec![(&PathId::ZERO, &3)]);
    }

    #[cfg(feature = "ack-inline-storage")]
    #[test]
    fn acked_paths_use_hash_map_for_multipath_and_replace_entries() {
        let mut paths = AckedPaths::default();
        let alternate = PathId(1);
        paths.insert(PathId::ZERO, 7);
        paths.insert(alternate, 9);
        paths.insert(PathId::ZERO, 13);

        assert!(!paths.is_inline_single_path());
        let mut values = paths
            .iter()
            .map(|(path, packet)| (*path, *packet))
            .collect::<Vec<_>>();
        values.sort_by_key(|(path, _)| *path);
        assert_eq!(values, vec![(PathId::ZERO, 13), (alternate, 9)]);

        paths.insert(alternate, 2);
        let mut values = paths
            .iter()
            .map(|(path, packet)| (*path, *packet))
            .collect::<Vec<_>>();
        values.sort_by_key(|(path, _)| *path);
        assert_eq!(values, vec![(PathId::ZERO, 13), (alternate, 2)]);
    }

    #[test]
    fn acked_paths_layout_is_bounded_and_reported() {
        let acked_paths = std::mem::size_of::<LargestAcked>();
        let sent_frames = std::mem::size_of::<SentFrames>();
        let sent_packet = std::mem::size_of::<spaces::SentPacket>();
        eprintln!(
            "ack-inline-storage layout: AckedPaths={acked_paths} SentFrames={sent_frames} SentPacket={sent_packet}"
        );
        assert!(acked_paths <= std::mem::size_of::<FxHashMap<PathId, u64>>() + 16);
    }
