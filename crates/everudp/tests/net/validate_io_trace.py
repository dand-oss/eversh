"""Validate QUIC and local terminal-edge I/O diagnostics.

The schema remains ``quic_io`` version 1.  The terminal stages are an
append-only extension in the same bounded sidecar; they are client-only and
intentionally have null connection and stream identifiers.
"""
...
TERMINAL = {"stdin_ready", "stdin_read_start", "stdin_read_end", "stdin_data",
            "stdin_dispatch"}
STAGES = CONTEXTUAL | {"udp_receive", "receive_copy", "driver_poll", "transmit_poll",
                      "transmit_accepted", "transmit_blocked", "transmit_error"} | TERMINAL
...
def validate(sidecar, base, role):
    ...
    for event in events:
        ...
        if stage in TERMINAL and role != "client":
            raise ValueError("terminal I/O stage is client-only")
