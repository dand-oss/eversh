#!/usr/bin/env bash
# Frozen single-path scheduling screen; not qualification.
set -Eeuo pipefail
source_dir=$1
expected=$2
prefix=$3
...
for mode in A B; do
    ...
    if [[ $mode == B ]]; then
        features='["cli","application-task-spike","stream-delivery-spike","quic-ack-threshold-spike","single-path-scheduling-spike"]'
    fi
...
for cell in 0 1; do
    ...
    for block in 0 1 2 3; do
        mode=${modes[$block]}
        seed=$((213300001 + 100 * cell + block))
        printf 'Single-path loss=%s block=%s mode=%s seed=%s\n' "$loss" "$block" "$mode" "$seed"
...
echo 'Frozen single-path comparison complete; qualification remains separate.'
