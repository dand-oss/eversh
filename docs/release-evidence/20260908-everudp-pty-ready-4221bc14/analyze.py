"""Reproduce one-shot PTY readiness screening; not release qualification."""
import hashlib
import json
import math
...
builds = {mode: json.loads((root / "builds" / mode / "provenance.json").read_text())
          for mode in "AB"}
expected = "4221bc1497a4fa0b1cfcbd688b47bd77785ebb55"
...
assert builds["B"]["everudp_build"]["cargo_features"] == features + ["pty-ready-spike"]
...
assert hashlib.sha256((root / "measurement/frozen-schedule.sh").read_bytes()).hexdigest() == (
    "d55decf2b2884d6a811d241db909a29af0a43d34cc9b36e54ec6e7cb55a41cc2")
...
for cell, loss in enumerate((0, 5)):
    ...
    for index, mode in enumerate("ABBA"):
        ...
        assert manifest["seeds"]["client"] == 212900001 + cell * 100 + index
