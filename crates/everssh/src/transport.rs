...
/// Linux route/address/link notifications without the everssh-specific route
/// supervisor policy. Consumers can combine this source with their own
/// reconnect lifetime and [`Endpoint::rebind`] decisions.
#[cfg(target_os = "linux")]
pub struct RouteChangeNotifications(linux_route::Notifications);

#[cfg(target_os = "linux")]
impl RouteChangeNotifications {
    pub fn open() -> std::io::Result<Self> {
        linux_route::Notifications::open().map(Self)
    }
