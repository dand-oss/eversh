...
/usr/bin/cc -O3 -Wall -Wextra -Werror -no-pie \
    -I "$OUTDIR/artifacts/include" -I "$ROOT/spikes/everudp/net" \
    "$ROOT/spikes/everudp/net/zmosh-bench.c" \
    "$ROOT/spikes/everudp/net/zmosh-transcript.c" \
...
/usr/bin/cc -std=c11 -O3 -Wall -Wextra -Werror \
    "$ROOT/spikes/everudp/net/pty-echo.c" \
    -o "$OUTDIR/artifacts/bin/pty-echo" \
    >"$OUTDIR/logs/pty-echo-build.stdout" \
    2>"$OUTDIR/logs/pty-echo-build.stderr"
...
artifact_paths = {
    ...
    "pty_echo": "artifacts/bin/pty-echo",
...
provenance = {
    ...
    "build": {
        ...
        "commands": [
            ...
            "cc -O3 -Wall -Wextra -Werror -no-pie zmosh-bench.c zmosh-transcript.c libzmosh.a -lpthread",
    ...
    "inputs": {
        ...
        "zmosh_transcript_source": {
            "sha256": digest(root / "spikes/everudp/net/zmosh-transcript.c")
        },
        "zmosh_transcript_header": {
            "sha256": digest(root / "spikes/everudp/net/zmosh-transcript.h")
        },
        "pty_echo_source": {
            "sha256": digest(root / "spikes/everudp/net/pty-echo.c")
        },
    },
    "artifacts": {
