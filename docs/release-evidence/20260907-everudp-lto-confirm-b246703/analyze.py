...
HARNESS_SHA = "b24670368f836fd1c52e60a7493ed5f24ea4a24d"
HARNESS_TREE = "7b30405d080120b1c3569edc6f780767ddd75389"
...
for loss in (0, 5):
    ...
    for index, role in enumerate(("baseline", "variant", "variant", "baseline"), 1):
        ...
        assert manifest["source"] == {"head_sha": HARNESS_SHA, "tree_sha": HARNESS_TREE, "dirty": False}
        record = (directory / "zmosh-quic/connect.log").read_text()
        assert re.fullmatch(r"ZMX_CONNECT quic [0-9]+ \[REDACTED\]\n", record)
    ...
    rng = random.Random(1151700 + loss)
...
print(json.dumps({"status": "DIAGNOSTIC", "qualification": False, "production_adoption": False,
    "source_sha": SHA, "tree_sha": TREE, "harness_sha": HARNESS_SHA,
    "harness_tree": HARNESS_TREE, "resamples": ITERATIONS, "cells": cells,
