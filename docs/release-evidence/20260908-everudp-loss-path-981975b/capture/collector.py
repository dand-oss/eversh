...
BUILD = Path('/tmp/everudp-loss-path-build-981975b')
OUT = Path('/tmp/everudp-loss-path-scheduler-981975b')
...
receipt = {'status': 'INVALID', 'production_qualification': False,
           'purpose': 'same-seed production path and scheduler correlation; not an uninstrumented comparison'}
...
try:
    ...
    require(provenance['everudp_build']['cargo_features'] == ['cli', 'path-diagnostics'],
            'isolated path diagnostic build required')
    ...
    write_json(OUT / 'plan.json', {'command': command, 'source': head,
        ...
        'affinity': '40,42,44,46', 'capture_seconds': 15,
        'scope': 'client and gateway main-thread scheduler events plus seven production boundary markers; broker/fixture scheduling not captured; no terminal payloads, argv exports or stacks'})
    ...
    with (OUT / 'benchmark.stdout').open('w') as stdout, (OUT / 'benchmark.stderr').open('w') as stderr:
        ...
        perf_command = [PERF, 'record', '-a', '--synth', 'no', '--clockid', 'mono',
            ...
            '-o', str(OUT / 'private-perf.data'), '--', '/usr/bin/sleep', '15']
