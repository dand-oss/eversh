...
A new `attach` client has a fresh authenticated association and no durable output position. New gateways start it at epoch zero without inheriting another attachment's GAP or retained output, regardless of which replay slot is reused. Updated clients still accept a validated initial GAP relative to that fresh position from older gateways without restarting them; this initialization is allowed only on first admission. Same-association network resume retains strict epoch continuity and duplicate suppression. Reattachment preserves the broker, PTY child, and application state; it does not restore discarded terminal history.
...
Public interface: `eversh [--remote-eversh WORD_OR_PATH] [--udp-port-range START:END] connect HOST [--session NAME] [--transport everssh|everudp|auto] [--take-over] [--ssh-option OPTION]... [-- COMMAND...]`, plus `attach`, `observe`, `list [--local-host NAME] [--json]`, `resume-all [--local-host NAME] [--transport ...]`, `detach`, `kill`, and `ssh HOST [-- SSH_OPTIONS [-- COMMAND]]`. `resume-all` lists matching live sessions, opens one Kitty tab per session (targeting `KITTY_LISTEN_ON`, keeping failed attaches visible until stdin closes, closing cleanly ended tabs), and reports every partial failure. Kitty integration stays in eversh.
