...
impl Connecting {
    ...
    #[cfg(feature = "inline-received-events")]
    pub(crate) fn weak_handle(&self) -> WeakConnectionHandle {
        self.conn
            .as_ref()
            .expect("connecting always owns its connection")
            .weak_handle()
    }
...
impl Future for ConnectionDriver {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        crate::diagnostic::emit(crate::diagnostic::Event::DriverPoll);
        let conn = &mut *self.0.lock_without_waking("poll");
        #[cfg(feature = "diagnostic-events")]
        crate::diagnostic::emit(crate::diagnostic::Event::DriverService {
            connection: conn.handle.0,
        });
        ...
        #[cfg(feature = "stream-delivery-handoff")]
        let handoff_was_deferred = std::mem::take(&mut conn.delivery_handoff_pending);
        #[cfg(feature = "stream-delivery-handoff")]
        if !handoff_was_deferred
            && conn.connected
            && conn.error.is_none()
            && !conn.blocked_readers.is_empty()
        {
            // Forward a readable event before doing queued transmit work when an application
            // reader was already blocked. This gives the reader a chance to consume data while
            // the state lock is released. The one-shot guard below guarantees that the next poll
            // performs normal transmit and timer work even if another readable event arrives.
            // Service expired deadlines before the optional cooperative handoff.
            let _ = conn.drive_timer(cx);
            conn.forward_endpoint_events();
            let reader_woken = conn.forward_app_events(&self.0.shared);
            if should_handoff_delivery(
                handoff_was_deferred,
                reader_woken,
                conn.connected,
                conn.error.is_some() || conn.inner.is_drained(),
            ) {
                conn.delivery_handoff_pending = true;
                // Register the driver before waking it. Dropping `conn` on return releases the
                // state lock; the self-wake then drives the guaranteed normal-service poll.
        ...
        #[cfg(feature = "inline-received-events")]
        let (_, inline_more) = conn.run_inline_datagram_handler();
        let mut keep_going = conn.drive_transmit(cx)?;
        #[cfg(feature = "inline-received-events")]
        {
            let pending_len = conn.inline_datagram_pending.as_ref().map(Bytes::len);
            keep_going |= inline_more
                || pending_len.is_some_and(|length| {
                    length <= conn.inner.datagrams().send_buffer_space()
                });
        }
        ...
        conn.forward_endpoint_events();
        let _ = conn.forward_app_events(&self.0.shared);
...
impl Connection {
    ...
    /// Installs a nonblocking application-DATAGRAM transform on the endpoint
    /// receive task. Each returned value is queued as one response DATAGRAM;
    /// returning `None` consumes the input without a response.
    ///
    /// This deliberately low-level hook is available only with
    /// `inline-received-events`. The callback runs while connection internals
    /// are locked, so it must not block, re-enter this connection, or panic.
    /// Installing a replacement drops the previous callback.
    #[cfg(feature = "inline-received-events")]
    pub fn set_inline_datagram_handler<F>(&self, handler: F)
    where
        F: FnMut(Bytes) -> Option<Bytes> + Send + 'static,
    {
        self.0
            .lock_without_waking("set_inline_datagram_handler")
            .inline_datagram_handler = Some(Box::new(handler));
    }
    ...
    /// Makes one immediate best-effort transmit pass on the caller's task.
    #[cfg(feature = "immediate-datagram-flush")]
    pub fn flush_transmit_now(&self, cx: &mut Context<'_>) -> io::Result<()> {
        let conn = &mut *self.0.lock_and_wake("flush_transmit_now");
        if conn.drive_transmit(cx)? {
            conn.wake();
        }
        Ok(())
    }
    ...
    /// Local endpoint connection handle used by packet diagnostic events.
    /// Unlike `stable_id`, this is not derived from an allocation address.
    /// Cache outside timed per-operation work: obtaining it takes the state lock.
    #[cfg(feature = "packet-diagnostics")]
    pub fn diagnostic_id(&self) -> usize {
        self.0.lock_without_waking("diagnostic_id").handle.0
    }
...
impl WeakConnectionHandle {
    ...
    /// Applies one endpoint-routed event on the endpoint task, after any
    /// already queued control event, and wakes the application before the
    /// ordinary connection driver resumes timer and transmit work.
    #[cfg(feature = "inline-received-events")]
    pub(crate) fn process_event_inline(
        &self,
        event: ConnectionEvent,
    ) -> Result<(), ConnectionEvent> {
        let Some(conn_ref) = self.upgrade_to_ref() else {
            return Err(event);
        };
        let conn = &mut *conn_ref.lock_without_waking("process_event_inline");
        if let Err(error) = conn.process_available_conn_events(&conn_ref.shared) {
            conn.terminate(error, &conn_ref.shared);
            conn.wake();
            return Ok(());
        }
        conn.process_conn_event(event, &conn_ref.shared);
        if !conn.connected {
            conn.wake();
            return Ok(());
        }
        conn.forward_endpoint_events();
        let _ = conn.forward_app_events(&conn_ref.shared);
        conn.run_inline_datagram_handler();
        // Transmission stays with ConnectionDriver, which propagates socket
        // errors. An endpoint-side flush must not consume a failed transmit
        // and then silently continue the connection.
        #[cfg(feature = "diagnostic-events")]
        crate::diagnostic::emit(crate::diagnostic::Event::InlineDriverWakeRequested {
            connection: conn.handle.0,
        });
        conn.wake();
        Ok(())
...
pub(crate) struct State {
    ...
    #[cfg(feature = "packet-diagnostics")]
    transmit_cookie: Option<u64>,
    #[cfg(feature = "stream-delivery-handoff")]
    delivery_handoff_pending: bool,
    #[cfg(feature = "inline-received-events")]
    inline_datagram_handler: Option<Box<dyn FnMut(Bytes) -> Option<Bytes> + Send>>,
    #[cfg(feature = "inline-received-events")]
    inline_datagram_pending: Option<Bytes>,
...
impl State {
    ...
    fn new(
        ...
        Self {
            ...
            #[cfg(feature = "packet-diagnostics")]
            transmit_cookie: None,
            #[cfg(feature = "stream-delivery-handoff")]
            delivery_handoff_pending: false,
            #[cfg(feature = "inline-received-events")]
            inline_datagram_handler: None,
            #[cfg(feature = "inline-received-events")]
            inline_datagram_pending: None,
        ...
        loop {
            ...
            let t = match self.buffered_transmit.take() {
                ...
                None => {
                    ...
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::ProtocolTransmitStart {
                        connection: self.handle.0,
                    });
                    #[cfg(feature = "packet-diagnostics")]
                    let protocol_transmit = {
                        self.transmit_cookie = crate::diagnostic::next_packet_cookie();
                        if let Some(cookie) = self.transmit_cookie {
                            proto::diagnostic::with_context(
                                proto::diagnostic::Context {
                                    connection: self.handle.0,
                                    cookie,
                                    direction: proto::diagnostic::Direction::Send,
                                },
                                || {
                            ...
                            crate::diagnostic::emit(crate::diagnostic::Event::PacketTraceInvalid);
                            self.inner
                    ...
                    let protocol_transmit =
                        self.inner
                            .poll_transmit(now, max_datagrams, &mut self.send_buffer);
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(match protocol_transmit.as_ref() {
                        Some(_) => crate::diagnostic::Event::ProtocolTransmitReady {
                            connection: self.handle.0,
                        },
                        None => crate::diagnostic::Event::ProtocolTransmitIdle {
                            connection: self.handle.0,
                        },
                    });
                    match protocol_transmit {
                        Some(t) => {
            ...
            let packet_event = |outcome| {
                if let Some(cookie) = self.transmit_cookie {
                    crate::diagnostic::emit(crate::diagnostic::Event::PacketTransmit {
                        connection: self.handle.0,
                        cookie,
                        bytes: len,
            ...
            #[cfg(feature = "packet-diagnostics")]
            packet_event(crate::diagnostic::TransmitOutcome::Poll);
            #[cfg(feature = "diagnostic-events")]
            crate::diagnostic::emit(crate::diagnostic::Event::TransmitPoll);
            match self
                .sender
                .as_mut()
                .poll_send(&udp_transmit(&t, &self.send_buffer[..len]), cx)
            {
                Poll::Pending => {
                    #[cfg(feature = "packet-diagnostics")]
                    packet_event(crate::diagnostic::TransmitOutcome::Blocked);
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::TransmitBlocked);
                    self.buffered_transmit = Some(t);
                    return Ok(false);
                }
                Poll::Ready(Err(e)) => {
                    #[cfg(feature = "packet-diagnostics")]
                    packet_event(crate::diagnostic::TransmitOutcome::Error);
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::TransmitError);
                    return Err(e);
                }
                Poll::Ready(Ok(())) => {
                    #[cfg(feature = "packet-diagnostics")]
                    packet_event(crate::diagnostic::TransmitOutcome::Accepted);
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::TransmitAccepted);
                }
            }
    ...
    #[cfg(feature = "inline-received-events")]
    fn run_inline_datagram_handler(&mut self) -> (bool, bool) {
        if self.inline_datagram_handler.is_none() {
            return (false, false);
        }
        let mut queued_response = false;
        // Bound endpoint-task work and retain one response under pressure.
        // The ordinary connection driver continues draining on its next poll.
        for _ in 0..8 {
            let response = match self.inline_datagram_pending.take() {
                Some(response) => Some(response),
                None => {
                    let Some(datagram) = self.inner.datagrams().recv() else {
                        return (queued_response, false);
                    };
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::InlineCallbackEnter {
                        connection: self.handle.0,
                    });
                    let response = self
                        .inline_datagram_handler
                        .as_mut()
                        .expect("checked above")(datagram);
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::InlineCallbackExit {
                        connection: self.handle.0,
                    });
                    response
                }
            ...
            if let Some(response) = response {
                match self.inner.datagrams().send(response, false) {
                    Ok(()) => {
                        #[cfg(feature = "diagnostic-events")]
                        crate::diagnostic::emit(crate::diagnostic::Event::InlineResponseQueued {
                            connection: self.handle.0,
                        });
                        queued_response = true
                    }
                    Err(proto::SendDatagramError::Blocked(response)) => {
                        self.inline_datagram_pending = Some(response);
                        #[cfg(feature = "diagnostic-events")]
                        crate::diagnostic::emit(crate::diagnostic::Event::InlineResponseBlocked {
                            connection: self.handle.0,
                        });
                        return (queued_response, false);
                    }
                    Err(_) => return (queued_response, false),
                }
            }
        }
        (queued_response, true)
    ...
    fn process_conn_events(
        ...
        loop {
            match self.conn_events.poll_recv(cx) {
                Poll::Ready(Some(event)) => self.process_conn_event(event, shared),
                Poll::Ready(None) => {
    ...
    #[cfg(feature = "inline-received-events")]
    fn process_available_conn_events(&mut self, shared: &Shared) -> Result<(), ConnectionError> {
        loop {
            match self.conn_events.try_recv() {
                Ok(event) => self.process_conn_event(event, shared),
    ...
    fn process_conn_event(&mut self, event: ConnectionEvent, shared: &Shared) {
        match event {
            ConnectionEvent::Rebind(sender) => {
                self.sender = sender;
            ...
            ConnectionEvent::Proto(event) => self.inner.handle_event(event),
            #[cfg(feature = "packet-diagnostics")]
            ConnectionEvent::ProtoPacket { event, cookie } => {
                proto::diagnostic::with_context(
                    proto::diagnostic::Context {
                        connection: self.handle.0,
                        cookie,
                        direction: proto::diagnostic::Direction::Receive,
                    },
                    || self.inner.handle_event(event),
                );
            }
            ConnectionEvent::Close { reason, error_code } => {
        ...
        while let Some(event) = self.inner.poll() {
            ...
            match event {
                ...
                Stream(StreamEvent::Writable { id }) => {
                    let _ = wake_stream(id, &mut self.blocked_writers);
                }
                ...
                Stream(StreamEvent::Readable { id }) => {
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::StreamReadable {
                        connection: self.handle.0,
                        stream: u64::from(id),
                    });
                    reader_woken |= wake_stream(id, &mut self.blocked_readers)
                }
...
fn wake_stream(stream_id: StreamId, wakers: &mut FxHashMap<StreamId, Waker>) -> bool {
...
#[cfg(feature = "stream-delivery-handoff")]
#[inline]
fn should_handoff_delivery(
    handoff_was_deferred: bool,
    reader_woken: bool,
    connected: bool,
    terminal: bool,
) -> bool {
    !handoff_was_deferred && reader_woken && connected && !terminal
}

#[cfg(all(test, feature = "stream-delivery-handoff"))]
mod delivery_handoff_tests {
    use super::should_handoff_delivery;

    #[test]
    fn only_an_actual_blocked_reader_wake_defers_once() {
        assert!(should_handoff_delivery(false, true, true, false));
        assert!(!should_handoff_delivery(true, true, true, false));
        assert!(!should_handoff_delivery(false, false, true, false));
        assert!(!should_handoff_delivery(false, true, false, false));
        assert!(!should_handoff_delivery(false, true, true, true));
    }
}
