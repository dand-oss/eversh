...
PARITY_PASS=0
if jq -e '.verdict.preregistered_point_rule_pass
        ...
        "$OUTROOT/analysis.json" >/dev/null; then
    PARITY_PASS=1
fi
FINISHED_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)

python3 - "$OUTROOT" "$HEAD_SHA" "$TREE_SHA" "$STARTED_UTC" \
    "$FINISHED_UTC" "$TRIALS" "$PARITY_PASS" <<'PY'
import hashlib
import json
import sys
...
head, tree, started, finished, trials, passed = sys.argv[2:]
...
receipt = {
    ...
    "qualification_outcome": "PASS" if passed == "1" else "FAIL",
}
...
if (( PARITY_PASS )); then
    echo "parity qualification PASS: $OUTROOT"
else
    echo "parity qualification FAIL (sealed evidence retained): $OUTROOT" >&2
    exit 1
fi
