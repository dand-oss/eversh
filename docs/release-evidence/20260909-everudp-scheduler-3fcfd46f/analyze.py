...
harness = "3fcfd46fd328c098405691928b82deec3dc96abf"
...
events = [f"syscalls:sys_{edge}_{call}" for call in
          ...
          for edge in ("enter", "exit")] + ["raw_syscalls:sys_enter", "raw_syscalls:sys_exit", "sched:sched_switch"]
reports = []
for direction, seed in (("forward", 213100011), ("reverse", 213100012)):
    ...
    assert receipt["scheduler"] is True
    ...
    for name in order:
        ...
        assert capture["scheduler_events_file"] == "scheduler-events.json"
        ...
        assert all(line.startswith(("syscalls:", "raw_syscalls:", "sched:", "dummy:u", "# Tip:")) for line in attrs)
        attrs = [line for line in attrs if line.startswith(("syscalls:", "raw_syscalls:", "sched:"))]
        ...
        scheduler = json.loads((target / "scheduler-events.json").read_text())
        report = analyze(result, export, capture, scheduler)
...
failed = json.loads((root / "failed-first-attempt.json").read_text())
assert failed["status"] == "INVALID" and failed["seed"] == 213100001
print(json.dumps({"status": "DIAGNOSTIC", "qualification": False, "reports": [
    {"direction": r["direction"], "candidate": r["candidate"],
     "included": r["analysis"]["included_trials"], "exclusions": r["exclusions"],
     "median_durations_ns": r["analysis"]["median_durations_ns"],
     "offcpu_windows": sum(row.get("off_cpu_ns", 0) > 0 for row in r["analysis"]["rows"])}
