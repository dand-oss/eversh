...
| 7a | SFTP/SCP compatibility and no second OpenSSH operation | Real (transport compatibility) + policy | Transport: `crates/everssh/tests/net/test-openssh.sh` SFTP batch, modern SCP download, and forwarding sessions; terminal behavior: `crates/everssh/tests/net/test-migration.sh` payload-independent expiry; supervisor: raw mode never retries (row 6a) and `eversh ssh` is the only raw surface | SFTP/SCP/forwarding pass byte-exactly through the everssh ProxyCommand transport (Slice 5A does not interrupt in-progress transfers); payload-independent terminal expiry/cleanup is proven by the M3 network gate; the composed supervisor exposes no distinct SFTP/SCP verb and no restart path — raw mode is the single supervisor surface | Slice 5A transport PASS; M3 network PASS at `930a3d7`; supervisor policy PASS at `df2d82b` |
...
| 11 | B2 terminal fallback timeline | Real | `crates/eversh/tests/net/test-composed-netns.sh b2` | one monotonic boot-uptime clock (/proc/uptime); ledger records loss, connection-death detection, client-budget exhaustion (from the terminal transcript line's own boot-uptime stamp), a bounded renewed-lease-start interval, server-association release, restore ≥10s later, post-drain backoff, and first fresh ssh; zero ssh attempts during the configured 30s drain; first fresh attempt and post-drain backoff ≥29s after terminal; exactly one first-attempt probe and reattach; same broker and local terminal PID; local scrollback retained; old-association input never delivered; only future input arrives; bounded invocation count (≤6 fresh entries = probe+reattach triples); head/tree identity, eversh binary hash, and computed deltas retained; annotator process group provably reaped | PASS with monotonic event ledger at the closure SHA |
