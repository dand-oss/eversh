...
The separately reviewed `everudp` product in [everudp-v1.md](everudp-v1.md) is the sanctioned optional layer above these raw primitives: its bounded opaque delivery queues do not add terminal parsing, state, prediction, or retained history to the `everpty` or `everssh` data paths.
