"""Reproduce single-path scheduling screening; not release qualification."""
import hashlib
import json
import math
...
builds = {mode: json.loads((root / "builds" / mode / "provenance.json").read_text())
          for mode in "AB"}
expected = "e2498d89770715ebcdb700d8c72d86feebc85f58"
...
assert builds["B"]["everudp_build"]["cargo_features"] == features + ["single-path-scheduling-spike"]
...
assert hashlib.sha256((root / "measurement/frozen-schedule.sh").read_bytes()).hexdigest() == (
    "f55427745ea99f0d654fdf557b0673605cd63239947cbf722c484a8c73449f1f")
...
for cell, loss in enumerate((0, 5)):
    ...
    for index, mode in enumerate("ABBA"):
        ...
        assert manifest["seeds"]["client"] == 213300001 + cell * 100 + index
