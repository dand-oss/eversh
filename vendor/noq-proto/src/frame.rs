...
impl AckFrequency {
    /// Frame type plus four maximum-width QUIC variable-length integers.
...
mod test {
    ...
    #[test]
    fn ack_frequency_size_bound_fits_all_maximum_width_fields() {
        ...
        let mut buffer = [0u8; AckFrequency::SIZE_BOUND];
        let mut remaining = &mut buffer[..];
        original.encode(&mut remaining);
