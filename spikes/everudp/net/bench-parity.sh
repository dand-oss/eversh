...
for required in "$ZMOSH_PREFIX/include/zmosh/zmosh.h" \
    "$ZMOSH_PREFIX/lib/libzmosh.a" "$NET/zmosh-bench.c" \
    "$NET/zmosh-transcript.c" "$NET/zmosh-transcript.h" "$NET/pty-echo.c"; do
...
if [[ -n ${ZMOSH_BENCH_BIN:-} ]]; then
    ...
    /usr/bin/cc -O3 -Wall -Wextra -Werror -no-pie \
        -I "$ZMOSH_PREFIX/include" -I "$NET" "$NET/zmosh-bench.c" \
        "$NET/zmosh-transcript.c" \
        "$ZMOSH_PREFIX/lib/libzmosh.a" -o "$ZMOSH_BENCH" -lpthread
fi
if [[ -n ${PTY_ECHO_BIN:-} ]]; then
    [[ -x $PTY_ECHO_BIN ]] || { echo "missing executable: $PTY_ECHO_BIN" >&2; exit 1; }
    PTY_ECHO=$PTY_ECHO_BIN
else
    PTY_ECHO=$TMP/pty-echo
    /usr/bin/cc -std=c11 -O3 -Wall -Wextra -Werror "$NET/pty-echo.c" \
        -o "$PTY_ECHO"
fi
...
run_zmosh() {
    $IP netns exec "$SERVER_NS" /usr/bin/env -u ZMX_SESSION TERM=xterm-256color ZMX_DIR="$ZMX_DIR" \
        "$ZMOSH_BIN" run "$SESSION" "$PTY_ECHO" >/dev/null
    sleep 0.3
    $IP netns exec "$SERVER_NS" /usr/bin/env -u ZMX_SESSION TERM=xterm-256color ZMX_DIR="$ZMX_DIR" \
...
PTY_ECHO_SHA=$(sha256sum "$PTY_ECHO" | awk '{print $1}')
...
manifest = {
    ...
    "workload": "rotating one-byte printable input through the compiled raw-PTY echo fixture",
    ...
    "artifacts": {
        ...
        "pty_echo": {"source": "spikes/everudp/net/pty-echo.c", "sha256": "$PTY_ECHO_SHA"},
    },
    "results": {
