#!/usr/bin/env bash
# Frozen threshold-only ACK screening, not release qualification.
set -Eeuo pipefail
source_dir=/tmp/everudp-threshold-e786e0cf-source.WjJiMf
prefix=/tmp/everudp-threshold-e786e0cf
control=/tmp/everudp-delivery-abccb1c-baseline-build
output_root=$prefix-measure
expected=$(git -C "$source_dir" rev-parse 'e786e0cf^{commit}')
...
for cell in 0 1; do
    ...
    for block in 0 1 2 3; do
        mode=${modes[$block]}
        seed=$((212700001 + 100 * cell + block))
        printf 'Threshold comparison loss=%s block=%s mode=%s seed=%s\n' "$loss" "$block" "$mode" "$seed"
...
echo 'Frozen threshold comparison complete; qualification remains separate.'
