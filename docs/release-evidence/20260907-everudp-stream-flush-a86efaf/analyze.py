...
SHA = "a86efafe2daff03e895616a6e8db9fd58ea48ffa"
TREE = "d145fe5eb17f905ba4292ff5887357350d221ae0"
...
for role, build in builds.items():
    ...
    assert build["everudp_build"]["cargo_features"] == (
        ["cli"] if role == "baseline" else ["cli", "stream-flush-spike"])
...
reuse = builds["variant"]["control_reuse"]
origins = {}
for name in ("control", "runtime"):
    path = ROOT / reuse[f"{name}_input"]
    assert digest(path) == reuse[f"{name}_provenance_sha256"]
    origins[name] = load(path)
assert origins["control"] == builds["baseline"]
assert origins["runtime"]["source"] == builds["variant"]["source"]
assert origins["runtime"]["everudp_build"] == builds["variant"]["everudp_build"]
for name in ("cargo", "rustc"):
    assert origins["runtime"]["tools"][name] == builds["baseline"]["tools"][name]
assert origins["runtime"]["inputs"]["Cargo.lock"] == builds["baseline"]["inputs"]["Cargo.lock"]
assert set(builds["variant"]["artifacts"]) == set(builds["baseline"]["artifacts"])
for name, artifact in builds["variant"]["artifacts"].items():
...
for loss in (0, 5):
    ...
    rng = random.Random(1030700 + loss)
