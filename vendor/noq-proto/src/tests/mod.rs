...
#[test]
fn discarded_handshake_spaces_match_generic_transmit_path() {
    let mut pair = Pair::default();
    let (client, server) = pair.connect();
    let now = pair.time;
    pair.client_conn_mut(client)
        .assert_discarded_space_noop(now);
...
/// Delayed ACK configuration remains interoperable with bidirectional stream
/// traffic at the minimum MTU. This is not a short-packet regression oracle:
/// the connection fixture can emit ACK_FREQUENCY during initial setup.
#[test]
fn ack_frequency_one_ms_with_simultaneous_stream_traffic() {
    let _guard = subscribe();
    let mut transport = TransportConfig::default();
    ...
    transport
        .ack_frequency_config(Some(config))
    ...
    let client_stream = pair.streams(Client).open(Dir::Bi).expect("client stream");
    pair.send_stream(Client, client_stream)
        .write(b"control")
        .expect("client stream write");
    pair.send_stream(Client, client_stream)
        .finish()
        .expect("client stream finish");

    let server_stream = pair.streams(Server).open(Dir::Uni).expect("server stream");
    pair.send_stream(Server, server_stream)
        .write(&[0u8; 1024])
        .expect("server stream write");
    pair.send_stream(Server, server_stream)
        .finish()
        .expect("server stream finish");

    assert!(
        !pair.drive_bounded(512),
        "stream traffic should quiesce without a packet-builder panic"
    );
    assert_eq!(pair.stats(Client).frame_tx.ack_frequency, 1);
    assert_eq!(pair.stats(Server).frame_tx.ack_frequency, 1);
}
