...
impl<'a, 'b> PacketBuilder<'a, 'b> {
    ...
    pub(super) fn finish(
        ...
        let protection = {
            ...
            let packet_number = self.packet_number;
            let supported = self.path == PathId::ZERO;
            move |started| {
                if supported {
                    diagnostic::emit_packet_protection(packet_number, space, started);
                }
            }
        };
        ...
        #[cfg(feature = "packet-diagnostics")]
        {
            ...
            // encode_start is absolute within the complete transmit buffer,
            // including previous GSO segments and coalesced packets. Building
            // is not UDP acceptance; the runtime retains the context cookie
            // through any subsequent blocked sender polls.
            if self.path != PathId::ZERO {
                diagnostic::emit_unsupported_path(self.path.0);
            } else {
                diagnostic::emit_packet_built(self.packet_number, space, encode_start, packet_len);
                for frame in &self.sent_frames.stream_frames {
                    diagnostic::emit_stream_sent(
                        self.packet_number,
                        space,
                        frame.id.into(),
                        frame.offsets.start,
