#!/usr/bin/env bash
# Frozen ACK-policy screening, not release qualification.
set -Eeuo pipefail
source_dir=/tmp/everudp-ack-9dc0c0e9-source.gyaqpH
prefix=/tmp/everudp-ack-9dc0c0e9
control=/tmp/everudp-delivery-abccb1c-baseline-build
output_root=$prefix-measure
expected=$(git -C "$source_dir" rev-parse '9dc0c0e9^{commit}')
...
for mode in A B; do
    ...
    if [[ $mode == B ]]; then
        features='["cli","application-task-spike","stream-delivery-spike","quic-ack-coalescing-spike"]'
    fi
...
for cell in 0 1; do
    ...
    for block in 0 1 2 3; do
        mode=${modes[$block]}
        seed=$((212600001 + 100 * cell + block))
        printf 'ACK comparison loss=%s block=%s mode=%s seed=%s\n' "$loss" "$block" "$mode" "$seed"
...
echo 'Frozen ACK comparison complete; qualification remains separate.'
