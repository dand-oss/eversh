...
Neither eversh nor everudp provides terminal prediction or local echo. QUIC is a reliable ordered byte transport, not latency magic; everudp removes the post-bootstrap SSH data path and its extra processing, but interaction remains subject to network round-trip time.

## The three components

The accepted v1 core has three components. The optional fourth `everudp` component ships in v2 under the contract frozen in [plans/everudp-v1.md](plans/everudp-v1.md). Every reliability, security, fuzz, process, and application gate passed at the release candidate; the frozen zmosh-UDP median performance gate did not, and the release decision recorded in that contract's section 13 accepts the measured result rather than reinterpreting the gate.
...
V2 installs exactly four physical executables from the same reusable crates: standalone `everpty`, standalone `everssh`, standalone `everudp`, and the user-facing combined/multi-role `eversh` executable. The combined `eversh` binary contains all four libraries and private role dispatch, but persistent PTY brokers, per-session everudp gateways, and one-shot QUIC servers remain separate operating-system processes.
