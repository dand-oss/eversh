...
//! environment, or process exit. Runtime construction belongs to the chosen
//! transport edge; selection itself and the supervisor/everpty roles build
//! none.
...
pub enum Role {
    ...
    Everpty,
    /// Private dispatch to the everssh QUIC edge.
    Everssh,
    /// Private dispatch to the everudp direct QUIC edge.
    Everudp,
}
...
pub fn select_role<T: AsRef<str>>(args: &[T]) -> Role {
    match args.first().map(|a| a.as_ref()) {
        ...
        Some(EVERUDP_ROLE) => Role::Everudp,
        _ => Role::Supervisor,
    }
}
