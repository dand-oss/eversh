...
| 11 | B2 terminal fallback timeline | Real | `crates/eversh/tests/net/test-composed-netns.sh b2` | 405s total path loss spans client budget and server lease: no ssh spawn during outage; carried terminal failure waits configured 30s association drain before one probe/fresh reattach; same broker and local terminal PID; local scrollback retained; old-association input never delivered; only future input arrives; bounded invocation count | PASS (see receipt below) |
