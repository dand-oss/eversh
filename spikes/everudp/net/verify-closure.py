...
def verify_build_provenance(
    ...
    require(schema_version in {1, 2}, "build provenance schema mismatch")
    require(
    ...
    if schema_version >= 2:
        input_paths.update(
            {
                "zmosh_transcript_source": repository_root
                / "spikes/everudp/net/zmosh-transcript.c",
                "zmosh_transcript_header": repository_root
                / "spikes/everudp/net/zmosh-transcript.h",
                "pty_echo_source": repository_root / "spikes/everudp/net/pty-echo.c",
            }
        )
...
def main() -> None:
    ...
    if "pty_echo" in built_hashes:
        pty_echo_hashes = {
            "parity": parity_analysis["source"]["artifact_sha256"].get("pty_echo"),
        }
        require(
            set(pty_echo_hashes.values()) == {built_hashes["pty_echo"]},
            "PTY echo fixture differs from the fresh exact-source build",
        )
    require(
