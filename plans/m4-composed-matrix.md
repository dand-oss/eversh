...
| 11 | B2 terminal fallback timeline | Real | `crates/eversh/tests/net/test-composed-netns.sh b2` | one CLOCK_MONOTONIC-derived clock; ledger records loss, connection-death detection, client-budget exhaustion (from the terminal transcript line's own monotonic stamp), a bounded renewed-lease-start interval, server-association release, restore ≥10s later, post-drain backoff, and first fresh ssh; zero ssh attempts during the configured 30s drain; first fresh attempt and post-drain backoff ≥29s after terminal; exactly one first-attempt probe and reattach; same broker and local terminal PID; local scrollback retained; old-association input never delivered; only future input arrives; bounded invocation count (≤6 fresh entries = probe+reattach triples); head/tree identity and computed deltas retained; annotator process group provably reaped | PASS with monotonic event ledger at the closure SHA |
