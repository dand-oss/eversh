...
impl Drop for EndpointDriver {
    fn drop(&mut self) {
        ...
        #[cfg(feature = "inline-received-events")]
        endpoint.recv_state.connections.inline.clear();
    ...
    fn handle_events(&mut self, cx: &mut Context<'_>, shared: &Shared) -> bool {
        for _ in 0..IO_LOOP_BOUND {
            ...
            if event.is_draining() {
                ...
                #[cfg(feature = "inline-received-events")]
                self.recv_state.connections.inline.remove(&ch);
...
fn respond(
    ...
    let result = sender.as_mut().poll_send(
        &udp_transmit(&transmit, &response_buffer[..transmit.size]),
        &mut cx,
    );
    #[cfg(feature = "diagnostic-events")]
    match &result {
        Poll::Pending => crate::diagnostic::emit(crate::diagnostic::Event::TransmitBlocked),
        Poll::Ready(Ok(())) => crate::diagnostic::emit(crate::diagnostic::Event::TransmitAccepted),
        Poll::Ready(Err(_)) => crate::diagnostic::emit(crate::diagnostic::Event::TransmitError),
    }
...
struct ConnectionSet {
    ...
    #[cfg(feature = "inline-received-events")]
    inline: FxHashMap<ConnectionHandle, WeakConnectionHandle>,
...
impl ConnectionSet {
    fn insert(
        ...
        let connecting = Connecting::new(handle, conn, self.sender.clone(), recv, sender, runtime);
        #[cfg(feature = "inline-received-events")]
        self.inline.insert(handle, connecting.weak_handle());
        connecting
    }
    ...
    fn poll_socket(
        ...
        loop {
            match socket.poll_recv(cx, &mut iovs, &mut metas) {
                Poll::Ready(Ok(msgs)) => {
                    #[cfg(feature = "diagnostic-events")]
                    crate::diagnostic::emit(crate::diagnostic::Event::UdpReceive);
                    self.recv_limiter.record_work(msgs);
                    for (meta, buf) in metas.iter().zip(iovs.iter()).take(msgs) {
                        let mut data: BytesMut = buf[0..meta.len].into();
                        #[cfg(feature = "diagnostic-events")]
                        crate::diagnostic::emit(crate::diagnostic::Event::ReceiveCopy);
                        while !data.is_empty() {
                            let buf = data.split_to(meta.stride.min(data.len()));
                            #[cfg(feature = "packet-diagnostics")]
                            let packet_cookie = crate::diagnostic::next_packet_cookie();
                            #[cfg(feature = "packet-diagnostics")]
                            match packet_cookie {
                                Some(cookie) => crate::diagnostic::emit(
                                    crate::diagnostic::Event::DatagramReceived {
                                        cookie,
                                        bytes: buf.len(),
                                    },
                                ),
                                None => crate::diagnostic::emit(
                                    crate::diagnostic::Event::PacketTraceInvalid,
                                ),
                            }
                            let mut response_buffer = Vec::new();
                            ...
                            match endpoint.handle(
                                ...
                                Some(DatagramEvent::ConnectionEvent(handle, event)) => {
                                    ...
                                    #[cfg(feature = "packet-diagnostics")]
                                    let event = match packet_cookie {
                                        Some(cookie) => {
                                            ConnectionEvent::ProtoPacket { event, cookie }
                                        }
                                        None => ConnectionEvent::Proto(event),
                                    };
                                    #[cfg(not(feature = "packet-diagnostics"))]
                                    let event = ConnectionEvent::Proto(event);
                                    #[cfg(feature = "inline-received-events")]
                                    let event = if let Some(connection) =
                                        self.connections.inline.get(&handle)
                                    {
                                        connection.process_event_inline(event)
                                    } else {
                                        Err(event)
                                    }
                                    .err();
                                    #[cfg(not(feature = "inline-received-events"))]
