...
HARNESS_SHA, HARNESS_TREE = sys.argv[2:4]
assert all(len(value) == 40 and all(c in "0123456789abcdef" for c in value)
           for value in (HARNESS_SHA, HARNESS_TREE))
...
for role, build in builds.items():
    ...
    assert reuse["runtime_input"] == "provenance-inputs/runtime.json"
    assert reuse["control_input"] == "provenance-inputs/control.json"
    originals = {}
    for origin in ("runtime", "control"):
        original_path = ROOT / "provenance-inputs" / role / f"{origin}.json"
        assert digest(original_path) == reuse[f"{origin}_provenance_sha256"]
        originals[origin] = load(original_path)
        assert originals[origin]["source"]["clean"] is True
    assert originals["runtime"]["source"] == build["source"]
    assert originals["runtime"]["everudp_build"] == build["everudp_build"]
    assert originals["control"]["source"]["head_sha"] == SHAS["baseline"]
    assert originals["control"]["zmosh_source"] == build["zmosh_source"]
    for name, artifact in build["artifacts"].items():
        origin = "runtime" if name == "everudp-floor" else "control"
        assert artifact == originals[origin]["artifacts"][name]
        selected = (reuse["selected_runtime_artifact"] if origin == "runtime"
                    else reuse["selected_control_artifacts"][name])
        assert selected == {"source_build": origin, "source_path": artifact["path"],
...
cells = {}
for loss, seed_base in ((0, 990700), (5, 995700)):
    ...
    for index, role in enumerate(("baseline", "candidate", "candidate", "baseline"), 1):
        ...
        assert manifest["source"] == {"head_sha": HARNESS_SHA,
                                      "tree_sha": HARNESS_TREE, "dirty": False}
