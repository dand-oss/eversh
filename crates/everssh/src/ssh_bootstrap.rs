...
/// Run the same bounded, policy-checked OpenSSH bootstrap while leaving the
/// exact protected record format to a sibling protocol. The returned bytes
/// are zeroized on drop and Debug exposes only their length and overflow bit.
pub async fn acquire_bootstrap_bytes(
    plan: &SshPlan,
    maximum: usize,
    limits: &Limits,
...
async fn run_owned_ssh(
    ...
    let (stdout, stderr, status) = match gathered {
...
fn classify_ssh_failure(status: ExitStatus, stderr: &[u8]) -> Error {
    ...
    const POLICY: [&[u8]; 8] = [
        ...
        b"Bad owner or permissions",
        b"garbage at end of line",
        b"command-line line 0",
        b"Unable to negotiate with",
    ...
    // network unavailability; an unknown (including empty) diagnostic is a
    // terminal protocol failure instead of an infinite recovery loop.
...
mod tests {
    ...
    fn ssh_exit_255_separates_authentication_policy_and_unavailability() {
        ...
        for terminal in [
            b"Unable to negotiate with 192.0.2.1 port 22: no matching host key type found\n"
                .as_slice(),
            b"an unrecognized OpenSSH failure\n".as_slice(),
            b"".as_slice(),
        ] {
            let failed = ExitStatus::from_raw(255 << 8);
            assert!(
    ...
    #[test]
    fn installed_openssh_keeps_reachability_retryable_and_unknown_protocol_terminal() {
        let quiet_config = std::env::temp_dir().join(format!(
            "everssh-quiet-config-{}-{}",
            std::process::id(),
        ...
        std::fs::write(&quiet_config, "Host *\n    LogLevel QUIET\n").expect("quiet config");

        let refused = TcpListener::bind((Ipv4Addr::LOCALHOST, 0)).expect("refused port");
        let refused_port = refused.local_addr().expect("refused address").port();
        drop(refused);
        let refused_plan = SshPlan::new(
            Ipv4Addr::LOCALHOST.to_string(),
            refused_port.to_string(),
            vec![
                format!("-F{}", quiet_config.display()),
                "-oConnectTimeout=1".to_owned(),
                "-oConnectionAttempts=1".to_owned(),
            ],
        )
        .expect("refused plan");
        let refused_output = StdCommand::new(SSH_PROGRAM)
            .args(refused_plan.bootstrap_args())
            .env("LC_ALL", "C")
            .env("LANG", "C")
            .output()
            .expect("run installed OpenSSH against refused port");
        assert_eq!(refused_output.status.code(), Some(255));
        assert!(
            !refused_output.stderr.is_empty(),
        ...
        let timeout_port = timeout.local_addr().expect("timeout address").port();
        let timeout_server = thread::spawn(move || {
            let (_stream, _) = timeout.accept().expect("accept timeout client");
            thread::sleep(Duration::from_secs(2));
        });
        let timeout_plan = SshPlan::new(
            Ipv4Addr::LOCALHOST.to_string(),
            timeout_port.to_string(),
            vec!["-oConnectTimeout=1".to_owned()],
        )
        .expect("timeout plan");
        let timeout_output = StdCommand::new(SSH_PROGRAM)
            .args(timeout_plan.bootstrap_args())
            .env("LC_ALL", "C")
            .env("LANG", "C")
            .output()
            .expect("run installed OpenSSH against silent peer");
        ...
        let protocol_port = protocol.local_addr().expect("protocol address").port();
        ...
        let protocol_output = StdCommand::new(SSH_PROGRAM)
            .args(protocol_plan.bootstrap_args())
            .env("LC_ALL", "C")
            .env("LANG", "C")
            .output()
