//! Role-selection purity and runtime-isolation tests: every non-everssh
...
#[test]
fn non_everssh_roles_never_construct_a_runtime() {
    // The counter starts at zero in this test process; selecting every
    // non-everssh role and running M1 dispatch logic must keep it there.
    ...
    for args in [
        ...
        assert_ne!(role, Role::Everssh, "fixture must be non-everssh: {args:?}");
        // Supervisor/everpty dispatch performs no runtime construction.
    }
    assert_eq!(
        everssh::runtime::constructions(),
        before,
        "non-everssh roles must leave the runtime counter untouched"
    );
}
