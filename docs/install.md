...
`--udp-port-range START:END` is also a global flag: the remote binds its
UDP endpoint to the first free port in that inclusive range (at most 1024
ports) instead of a kernel-chosen one. Use it when the remote host firewall
only admits a fixed UDP range, for example a ufw rule allowing the mosh
range 60000:60010:

    eversh --remote-eversh /home/user/.local/bin/eversh --udp-port-range 60000:60010 list myhost

It applies to both transports and to every command that bootstraps
(`connect`, `attach`, `observe`, `list`, `resume-all`, `detach`, `kill`); the
standalone `everssh ssh-proxy` accepts the same option. For everudp, the
range governs the gateway a bootstrap starts; a session whose gateway is
already running keeps its port. The value is validated locally before any
SSH work. It needs eversh 0.2.3 or later on the remote host: an older remote
rejects the option and the bootstrap fails closed with a diagnostic naming
`--udp-port-range`, never falling back to a random port. Without the flag the
remote command is unchanged, so older remotes keep working.

## Use

    eversh [--remote-eversh WORD_OR_PATH] [--udp-port-range START:END] connect HOST [--session NAME] [--transport everssh|everudp|auto] [--take-over] [--ssh-option OPTION]... [-- COMMAND...]
    eversh [--remote-eversh WORD_OR_PATH] [--udp-port-range START:END] attach  HOST NAME [--transport everssh|everudp|auto] [--take-over] [--ssh-option OPTION]...
    eversh [--remote-eversh WORD_OR_PATH] [--udp-port-range START:END] observe HOST NAME [--transport everssh|everudp|auto] [--ssh-option OPTION]...
    eversh [--remote-eversh WORD_OR_PATH] [--udp-port-range START:END] list    HOST [--local-host NAME] [--json] [--ssh-option OPTION]...
