#!/usr/bin/env bash
# Bounded initial screen only. No release qualification.
set -Eeuo pipefail
source_dir=/tmp/everudp-discarded-screen-source.uOJOrf/repo
expected=9be1b79d25c5fb3347b9335c6899c9f3a7e1b932
prefix=/tmp/everudp-discarded-9be1b79d
...
for mode in A B; do
    ...
    features='["cli"]'
    [[ $mode == A ]] || features='["cli","discarded-space-spike"]'
...
for block in 0 1 2 3; do
    mode=${modes[$block]}
    seed=$((216000001 + block))
    printf 'Discarded-space screen block=%s mode=%s seed=%s\n' "$block" "$mode" "$seed"
...
echo 'Initial screen complete; qualification remains separate.'
