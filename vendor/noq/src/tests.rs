...
use super::{
    AckFrequencyConfig, ClientConfig, Endpoint, EndpointConfig, RecvStream, SendStream,
    TransportConfig,
};
...
/// The first application stream must remain deliverable when both peers
/// request delayed ACKs.  This is the smallest reproduction of the
/// write-before-accept contract: the client writes the control payload before
/// the server calls `accept_bi`, and the server must still observe and echo it
/// within a bounded deadline.
#[test]
fn ack_threshold_one_bidi_write_before_accept() {
    let _guard = subscribe();
    let runtime = rt_basic();
    ...
    let address = endpoint.local_addr().expect("endpoint address");
    let server = endpoint.clone();
    let payload = Bytes::from_static(b"ack-threshold-one-control");
    let expected = payload.clone();

    let server_task = runtime.spawn(async move {
        let incoming = tokio::time::timeout(Duration::from_secs(3), server.accept())
            .await
            .expect("server accept deadline")
            .expect("server endpoint");
        let connection = tokio::time::timeout(Duration::from_secs(3), incoming)
            .await
            .expect("server handshake deadline")
            .expect("server connection");
        let (mut send, mut recv) =
            tokio::time::timeout(Duration::from_secs(3), connection.accept_bi())
                .await
                .expect("server accept_bi deadline")
                .expect("server bidirectional stream");
        let received = tokio::time::timeout(Duration::from_secs(3), recv.read_to_end(1024))
            .await
            .expect("server read deadline")
            .expect("server read");
        send.write_all(&received).await.expect("server echo");
        send.finish().expect("server finish");
        let _ = tokio::time::timeout(Duration::from_secs(3), send.stopped())
            .await
            .expect("server echo acknowledgement deadline");
        received
    });
    ...
    let result = runtime.block_on(async move {
        ...
        let (mut send, mut recv) =
            tokio::time::timeout(Duration::from_secs(3), connection.open_bi())
                .await
                .expect("client open_bi deadline")
                .expect("client bidirectional stream");
        send.write_all(&payload).await.expect("client write");
        send.finish().expect("client finish");
        let echoed = tokio::time::timeout(Duration::from_secs(3), recv.read_to_end(1024))
            .await
    ...
    let server_received = runtime.block_on(server_task).expect("server task");
    assert_eq!(server_received, b"ack-threshold-one-control");
    assert_eq!(result, b"ack-threshold-one-control");
}
