#[cfg(any(feature = "runtime-tokio", feature = "runtime-smol"))]
use std::sync::Arc;
use std::{
    fmt::{self, Debug},
    future::Future,
    io::{self, IoSliceMut},
    net::SocketAddr,
    num::NonZeroUsize,
    pin::Pin,
    task::{Context, Poll},
};

use udp::{RecvMeta, Transmit};

use crate::Instant;

/// Abstracts I/O and timer operations for runtime independence
pub trait Runtime: Send + Sync + Debug + 'static {
    /// Construct a timer that will expire at `i`
    fn new_timer(&self, i: Instant) -> Pin<Box<dyn AsyncTimer>>;
    /// Drive `future` to completion in the background
    #[track_caller]
    fn spawn(&self, future: Pin<Box<dyn Future<Output = ()> + Send>>);
    /// Convert `t` into the socket type used by this runtime
    #[cfg(not(wasm_browser))]
    fn wrap_udp_socket(&self, t: std::net::UdpSocket) -> io::Result<Box<dyn AsyncUdpSocket>>;
    /// Look up the current time
    ///
    /// Allows simulating the flow of time for testing.
    fn now(&self) -> Instant {
        Instant::now()
    }
}

/// Abstract implementation of an async timer for runtime independence
pub trait AsyncTimer: Send + Debug + 'static {
    /// Update the timer to expire at `i`
    fn reset(self: Pin<&mut Self>, i: Instant);
    /// Check whether the timer has expired, and register to be woken if not
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<()>;
}

/// Abstract implementation of a UDP socket for runtime independence
pub trait AsyncUdpSocket: Send + Sync + Debug + 'static {
    /// Create a [`UdpSender`] that can register a single task for write-readiness notifications
    /// and send a transmit, if ready.
    ///
    /// A `poll_send` method on a single object can usually store only one [`Waker`] at a time,
    /// i.e. allow at most one caller to wait for an event. This method allows any number of
    /// interested tasks to construct their own [`UdpSender`] object. They can all then wait for the
    /// same event and be notified concurrently, because each [`UdpSender`] can store a separate
    /// [`Waker`].
    ///
    /// [`Waker`]: std::task::Waker
    fn create_sender(&self) -> Pin<Box<dyn UdpSender>>;

    /// Receive UDP datagrams, or register to be woken if receiving may succeed in the future
    fn poll_recv(
        &mut self,
        cx: &mut Context<'_>,
        bufs: &mut [IoSliceMut<'_>],
        meta: &mut [RecvMeta],
    ) -> Poll<io::Result<usize>>;

    /// Look up the local IP address and port used by this socket
    fn local_addr(&self) -> io::Result<SocketAddr>;

    /// Maximum number of datagrams that might be described by a single [`RecvMeta`]
    fn max_receive_segments(&self) -> NonZeroUsize {
        NonZeroUsize::MIN
    }

    /// Whether datagrams might get fragmented into multiple parts
    ///
    /// Sockets should prevent this for best performance. See e.g. the `IPV6_DONTFRAG` socket
    /// option.
    fn may_fragment(&self) -> bool {
        true
    }
}

/// An object for asynchronously writing to an associated [`AsyncUdpSocket`].
///
/// Any number of [`UdpSender`]s may exist for a single [`AsyncUdpSocket`]. Each [`UdpSender`] is
/// responsible for notifying at most one task for send readiness.
pub trait UdpSender: Send + Sync + Debug + 'static {
    /// Send a UDP datagram, or register to be woken if sending may succeed in the future.
    ///
    /// Usually implementations of this will poll the socket for writability before trying to
    /// write to them, and retry both if writing fails.
    ///
    /// noq will create multiple [`UdpSender`]s, one for each task it's using it from. Thus it's
    /// important to poll the underlying socket in a way that doesn't overwrite wakers.
    ///
    /// A single [`UdpSender`] will be reused, even if `poll_send` returns `Poll::Ready` once,
    /// unlike [`Future::poll`], so calling it again after readiness should not panic.
    fn poll_send(
        self: Pin<&mut Self>,
        transmit: &Transmit<'_>,
        cx: &mut Context<'_>,
    ) -> Poll<io::Result<()>>;

    /// Maximum number of datagrams that a [`Transmit`] may encode.
    fn max_transmit_segments(&self) -> NonZeroUsize {
        NonZeroUsize::MIN
    }
}

pin_project_lite::pin_project! {
    /// A helper for constructing [`UdpSender`]s from an underlying `Socket` type.
    ///
    /// This struct implements [`UdpSender`] if `MakeWritableFn` produces a `WritableFut`.
    ///
    /// Also serves as a trick, since `WritableFut` doesn't need to be a named future,
    /// it can be an anonymous async block, as long as `MakeWritableFn` produces that
    /// anonymous async block type.
    ///
    /// The `UdpSenderHelper` generic type parameters don't need to named, as it will be
    /// used in its dyn-compatible form as a `Pin<Box<dyn UdpSender>>`.
    struct UdpSenderHelper<Socket, MakeWritableFutFn, WritableFut> {
        socket: Socket,
        make_writable_fut_fn: MakeWritableFutFn,
        #[pin]
        writable_fut: Option<WritableFut>,
    }
}

impl<Socket, MakeWritableFutFn, WritableFut> Debug
    for UdpSenderHelper<Socket, MakeWritableFutFn, WritableFut>
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("UdpSender")
    }
}

impl<Socket, MakeWritableFutFn, WriteableFut>
    UdpSenderHelper<Socket, MakeWritableFutFn, WriteableFut>
{
    /// Create helper that implements [`UdpSender`] from a socket.
    ///
    /// Additionally you need to provide what is essentially an async function
    /// that resolves once the socket is write-ready.
    ///
    /// See also the bounds on this struct's [`UdpSender`] implementation.
    #[cfg(any(feature = "runtime-smol", feature = "runtime-tokio",))]
    fn new(inner: Socket, make_fut: MakeWritableFutFn) -> Self {
        Self {
            socket: inner,
            make_writable_fut_fn: make_fut,
            writable_fut: None,
        }
    }
}

impl<Socket, MakeWritableFutFn, WritableFut> UdpSender
    for UdpSenderHelper<Socket, MakeWritableFutFn, WritableFut>
where
    Socket: UdpSenderHelperSocket,
    MakeWritableFutFn: Fn(&Socket) -> WritableFut + Send + Sync + 'static,
    WritableFut: Future<Output = io::Result<()>> + Send + Sync + 'static,
{
    fn poll_send(
        self: Pin<&mut Self>,
        transmit: &Transmit<'_>,
        cx: &mut Context<'_>,
    ) -> Poll<io::Result<()>> {
        let mut this = self.project();
        // The experiment makes one nonblocking attempt only when no readiness future is
        // stored. A WouldBlock falls through to the existing future/waker loop; a pending
        // future is never bypassed and the existing retry behavior remains unchanged.
        #[cfg(feature = "udp-send-fast-path")]
        if this.writable_fut.is_none() {
            match this.socket.try_send(transmit) {
                Err(e) if e.kind() == io::ErrorKind::WouldBlock => {}
                result => return Poll::Ready(result),
            }
        }
        loop {
            if this.writable_fut.is_none() {
                this.writable_fut
                    .set(Some((this.make_writable_fut_fn)(this.socket)));
            }
            // We're forced to `unwrap` here because `Fut` may be `!Unpin`, which means we can't safely
            // obtain an `&mut WritableFut` after storing it in `self.writable_fut` when `self` is already behind `Pin`,
            // and if we didn't store it then we wouldn't be able to keep it alive between
            // `poll_send` calls.
            let result =
                std::task::ready!(this.writable_fut.as_mut().as_pin_mut().unwrap().poll(cx));

            // Polling an arbitrary `Future` after it becomes ready is a logic error, so arrange for
            // a new `Future` to be created on the next call.
            this.writable_fut.set(None);

            // If .writable() fails, propagate the error
            result?;

            match this.socket.try_send(transmit) {
                // We thought the socket was writable, but it wasn't, then retry so that either another
                // `writable().await` call determines that the socket is indeed not writable and
                // registers us for a wakeup, or the send succeeds if this really was just a
                // transient failure.
                Err(e) if e.kind() == io::ErrorKind::WouldBlock => continue,
                // In all other cases, either propagate the error or we're Ok
                result => return Poll::Ready(result),
            }
        }
    }

    fn max_transmit_segments(&self) -> NonZeroUsize {
        self.socket.max_transmit_segments()
    }
}

/// Parts of the [`UdpSender`] trait that aren't asynchronous or require storing wakers.
///
/// This trait is used by [`UdpSenderHelper`] to help construct [`UdpSender`]s.
trait UdpSenderHelperSocket: Send + Sync + 'static {
    /// Try to send a transmit, if the socket happens to be write-ready.
    ///
    /// If not write-ready, this is allowed to return [`std::io::ErrorKind::WouldBlock`].
    ///
    /// The [`UdpSenderHelper`] will use this to implement [`UdpSender::poll_send`].
    fn try_send(&self, transmit: &Transmit<'_>) -> io::Result<()>;

    /// See [`UdpSender::max_transmit_segments`].
    fn max_transmit_segments(&self) -> NonZeroUsize;
}

/// Automatically select an appropriate runtime from those enabled at compile time
///
/// If `runtime-tokio` is enabled and this function is called from within a Tokio runtime context,
/// then `TokioRuntime` is returned. Otherwise, if `runtime-smol` is enabled, `SmolRuntime` is
/// returned. Otherwise, `None` is returned.
#[cfg(any(feature = "runtime-tokio", feature = "runtime-smol"))]
#[allow(clippy::needless_return)] // Be sure we return the right thing
pub fn default_runtime() -> Option<Arc<dyn Runtime>> {
    #[cfg(feature = "runtime-tokio")]
    {
        if ::tokio::runtime::Handle::try_current().is_ok() {
            return Some(Arc::new(TokioRuntime));
        }
    }

    #[cfg(feature = "runtime-smol")]
    {
        return Some(Arc::new(SmolRuntime));
    }

    #[cfg(not(feature = "runtime-smol"))]
    None
}

#[cfg(feature = "runtime-tokio")]
mod tokio;
#[cfg(feature = "runtime-tokio")]
pub use tokio::TokioRuntime;

#[cfg(feature = "runtime-smol")]
mod smol;
#[cfg(feature = "runtime-smol")]
pub use smol::*;

#[cfg(all(test, any(feature = "runtime-tokio", feature = "runtime-smol")))]
mod tests {
    use super::*;
    use std::{
        collections::VecDeque,
        sync::{
            Arc, Mutex,
            atomic::{AtomicUsize, Ordering},
        },
        task::{Context, Poll},
    };

    #[derive(Debug, Default)]
    struct FakeSocketState {
        sends: VecDeque<io::Result<()>>,
        send_calls: usize,
        sent_payloads: Vec<Vec<u8>>,
        writable_futures: usize,
    }

    #[derive(Debug)]
    struct FakeSocket {
        state: Arc<Mutex<FakeSocketState>>,
        writable: Arc<Mutex<WritableState>>,
    }

    impl UdpSenderHelperSocket for FakeSocket {
        fn try_send(&self, transmit: &udp::Transmit<'_>) -> io::Result<()> {
            let mut state = self.state.lock().unwrap();
            state.send_calls += 1;
            let result = state.sends.pop_front().unwrap_or(Ok(()));
            if result.is_ok() {
                state.sent_payloads.push(transmit.contents.to_vec());
            }
            result
        }

        fn max_transmit_segments(&self) -> NonZeroUsize {
            NonZeroUsize::MIN
        }
    }

    fn make_writable(socket: &FakeSocket) -> WritableFuture {
        socket.state.lock().unwrap().writable_futures += 1;
        WritableFuture(socket.writable.clone())
    }

    #[derive(Debug)]
    struct WritableState {
        pending: bool,
        result: Option<io::ErrorKind>,
        polls: usize,
        waker: Option<std::task::Waker>,
    }

    #[derive(Debug)]
    struct WritableFuture(Arc<Mutex<WritableState>>);

    impl Future for WritableFuture {
        type Output = io::Result<()>;

        fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
            let mut state = self.0.lock().unwrap();
            state.polls += 1;
            if state.pending {
                state.waker = Some(cx.waker().clone());
                Poll::Pending
            } else {
                Poll::Ready(match state.result {
                    None => Ok(()),
                    Some(kind) => Err(io::Error::new(kind, "fake writable error")),
                })
            }
        }
    }

    fn transmit() -> udp::Transmit<'static> {
        transmit_with(b"test")
    }

    fn transmit_with(contents: &'static [u8]) -> udp::Transmit<'static> {
        udp::Transmit {
            destination: "127.0.0.1:4433".parse().unwrap(),
            ecn: None,
            contents,
            segment_size: None,
            src_ip: None,
        }
    }

    struct CountWake(Arc<AtomicUsize>);

    impl std::task::Wake for CountWake {
        fn wake(self: Arc<Self>) {
            self.0.fetch_add(1, Ordering::SeqCst);
        }

        fn wake_by_ref(self: &Arc<Self>) {
            self.0.fetch_add(1, Ordering::SeqCst);
        }
    }

    fn context() -> Context<'static> {
        Context::from_waker(std::task::Waker::noop())
    }

    fn make_sender(
        sends: impl IntoIterator<Item = io::Result<()>>,
        writable: Arc<Mutex<WritableState>>,
    ) -> (
        Pin<Box<UdpSenderHelper<FakeSocket, fn(&FakeSocket) -> WritableFuture, WritableFuture>>>,
        Arc<Mutex<FakeSocketState>>,
    ) {
        let socket_state = Arc::new(Mutex::new(FakeSocketState {
            sends: sends.into_iter().collect(),
            ..FakeSocketState::default()
        }));
        let socket = FakeSocket {
            state: socket_state.clone(),
            writable,
        };
        let make_writable_fn: fn(&FakeSocket) -> WritableFuture = make_writable;
        let sender = UdpSenderHelper::new(socket, make_writable_fn);
        (Box::pin(sender), socket_state)
    }

    #[test]
    fn immediate_success_uses_fast_path_only_when_enabled() {
        let writable = Arc::new(Mutex::new(WritableState {
            pending: false,
            result: None,
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) = make_sender([Ok(())], writable);
        let result = UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context());
        assert!(matches!(result, Poll::Ready(Ok(()))));
        let state = socket.lock().unwrap();
        assert_eq!(state.send_calls, 1);
        #[cfg(feature = "udp-send-fast-path")]
        assert_eq!(state.writable_futures, 0);
        #[cfg(not(feature = "udp-send-fast-path"))]
        assert_eq!(state.writable_futures, 1);
    }

    #[test]
    fn would_block_then_pending_then_success() {
        let writable = Arc::new(Mutex::new(WritableState {
            pending: true,
            result: None,
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) = make_sender(
            [Err(io::Error::from(io::ErrorKind::WouldBlock)), Ok(())],
            writable.clone(),
        );
        let wake_count = Arc::new(AtomicUsize::new(0));
        let wake_waker = std::task::Waker::from(Arc::new(CountWake(wake_count.clone())));
        let mut wake_cx = Context::from_waker(&wake_waker);
        let first = UdpSender::poll_send(sender.as_mut(), &transmit(), &mut wake_cx);
        assert!(matches!(first, Poll::Pending));
        #[cfg(feature = "udp-send-fast-path")]
        assert_eq!(socket.lock().unwrap().send_calls, 1);
        #[cfg(not(feature = "udp-send-fast-path"))]
        assert_eq!(socket.lock().unwrap().send_calls, 0);
        let registered_waker = {
            let mut state = writable.lock().unwrap();
            state.pending = false;
            state.waker.take().expect("pending future registers waker")
        };
        registered_waker.wake();
        assert_eq!(wake_count.load(Ordering::SeqCst), 1);
        let second = UdpSender::poll_send(sender.as_mut(), &transmit(), &mut wake_cx);
        assert!(matches!(second, Poll::Ready(Ok(()))));
        assert_eq!(socket.lock().unwrap().send_calls, 2);
    }

    #[test]
    fn pending_future_is_not_bypassed_by_speculative_send() {
        let writable = Arc::new(Mutex::new(WritableState {
            pending: true,
            result: None,
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) = make_sender(
            [Err(io::Error::from(io::ErrorKind::WouldBlock)), Ok(())],
            writable.clone(),
        );
        assert!(matches!(
            UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context()),
            Poll::Pending
        ));
        assert!(matches!(
            UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context()),
            Poll::Pending
        ));
        #[cfg(feature = "udp-send-fast-path")]
        assert_eq!(socket.lock().unwrap().send_calls, 1);
        #[cfg(not(feature = "udp-send-fast-path"))]
        assert_eq!(socket.lock().unwrap().send_calls, 0);
        writable.lock().unwrap().pending = false;
        assert!(matches!(
            UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context()),
            Poll::Ready(Ok(()))
        ));
        assert_eq!(socket.lock().unwrap().send_calls, 2);
    }

    #[test]
    fn spurious_readiness_retries_without_duplication() {
        let writable = Arc::new(Mutex::new(WritableState {
            pending: false,
            result: None,
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) = make_sender(
            [
                Err(io::Error::from(io::ErrorKind::WouldBlock)),
                Err(io::Error::from(io::ErrorKind::WouldBlock)),
                Ok(()),
            ],
            writable,
        );
        assert!(matches!(
            UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context()),
            Poll::Ready(Ok(()))
        ));
        assert_eq!(socket.lock().unwrap().send_calls, 3);
        assert_eq!(socket.lock().unwrap().sent_payloads, vec![b"test".to_vec()]);
    }

    #[test]
    fn repeated_sends_preserve_payload_order_without_duplication() {
        let writable = Arc::new(Mutex::new(WritableState {
            pending: false,
            result: None,
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) = make_sender([Ok(()), Ok(())], writable);
        assert!(matches!(
            UdpSender::poll_send(sender.as_mut(), &transmit_with(b"one"), &mut context()),
            Poll::Ready(Ok(()))
        ));
        assert!(matches!(
            UdpSender::poll_send(sender.as_mut(), &transmit_with(b"two"), &mut context()),
            Poll::Ready(Ok(()))
        ));
        assert_eq!(
            socket.lock().unwrap().sent_payloads,
            vec![b"one".to_vec(), b"two".to_vec()]
        );
    }

    #[test]
    fn readiness_and_retry_errors_are_returned() {
        let writable = Arc::new(Mutex::new(WritableState {
            pending: false,
            result: Some(io::ErrorKind::PermissionDenied),
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) =
            make_sender([Err(io::Error::from(io::ErrorKind::WouldBlock))], writable);
        let result = UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context());
        assert!(
            matches!(result, Poll::Ready(Err(error)) if error.kind() == io::ErrorKind::PermissionDenied)
        );
        #[cfg(feature = "udp-send-fast-path")]
        assert_eq!(socket.lock().unwrap().send_calls, 1);
        #[cfg(not(feature = "udp-send-fast-path"))]
        assert_eq!(socket.lock().unwrap().send_calls, 0);

        let writable = Arc::new(Mutex::new(WritableState {
            pending: false,
            result: None,
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) = make_sender(
            [
                Err(io::Error::from(io::ErrorKind::WouldBlock)),
                Err(io::Error::from(io::ErrorKind::PermissionDenied)),
            ],
            writable,
        );
        let result = UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context());
        assert!(
            matches!(result, Poll::Ready(Err(error)) if error.kind() == io::ErrorKind::PermissionDenied)
        );
        assert_eq!(socket.lock().unwrap().send_calls, 2);
    }

    #[cfg(feature = "udp-send-fast-path")]
    #[test]
    fn initial_non_would_block_error_does_not_create_readiness() {
        let writable = Arc::new(Mutex::new(WritableState {
            pending: false,
            result: None,
            polls: 0,
            waker: None,
        }));
        let (mut sender, socket) = make_sender(
            [Err(io::Error::from(io::ErrorKind::PermissionDenied))],
            writable,
        );
        let result = UdpSender::poll_send(sender.as_mut(), &transmit(), &mut context());
        assert!(
            matches!(result, Poll::Ready(Err(error)) if error.kind() == io::ErrorKind::PermissionDenied)
        );
        assert_eq!(socket.lock().unwrap().send_calls, 1);
        assert_eq!(socket.lock().unwrap().writable_futures, 0);
    }
}
